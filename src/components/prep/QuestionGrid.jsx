...
const QuestionGrid = ({ questions, answers, currentIndex, onNavigate, isOpen = true, onClose }) => {
  const PAGE_SIZE = 20;
  ...
  const getQuestionStatus = (question) => (answers[question.id] ? "answered" : "unanswered");
  ...
  const content = (
    <div className="h-full flex flex-col bg-[#ffffff] font-inter">
      <div className="p-6 border-b border-[#c4c6cf]/15">
        ...
        <p className="text-sm text-[#000a1e]/60">Batch {page + 1} of {totalPages}</p>
      </div>
      ...
      {totalPages > 1 && (
        <div className="p-4 flex justify-between gap-2 border-t border-[#c4c6cf]/15">
          ...
          <span className="text-xs font-bold flex items-center">Questions {startIdx + 1} - {Math.min(startIdx + PAGE_SIZE, questions.length)}</span>
          <button 
  ...
  return (
    <div className={`${onClose ? (isOpen ? "fixed inset-0 z-50 flex justify-end" : "hidden") : "hidden md:block h-fit sticky top-36"}`}>
      {onClose && <div className="absolute inset-0 bg-[#000a1e]/20 backdrop-blur-sm" onClick={onClose} />}
      <div className={`relative w-80 shadow-2xl h-full md:h-auto md:rounded-2xl overflow-hidden ${onClose ? "animate-slide-left" : "bg-white"}`}>
        {content}
      </div>
    </div>
  );
